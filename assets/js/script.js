...
function fetchRecipe(ingredients, ranking, ignorePantry) {
  console.log(ingredients, ranking, ignorePantry);
...
function displaySelectedIngredients() {
	const selectedIngredientContainer = document.querySelector(
		".selectedIngredients"
	);
	...
	if (selectedIngredients.length === 1) {
		selectYourIngredient.textContent =
			"You have selected the below ingredient. Add more ingredients!";
		...
		selectYourIngredient.textContent =
			"Add more ingredients to refine your search for recipes.";
		noRecipeMessage.textContent = "";
		selectYourIngredient.style.color = "darkblue";
	} else {
		selectYourIngredient.textContent =
			"\u2713 the box to get recipe based on Selected Ingredients";
...
btnEl.addEventListener("click", function () {
  ...
  fetchRecipe(storedIngredients);
}

	let userInput = selectedIngredients.join(", ");
	let includeSelectedOnly = includeSelectedCheckbox.checked;
	let ranking = 1;
...
includeSelectedCheckbox.addEventListener("change", function () {
	let ignorePantryMessage = document.querySelector(".ignorePantryMessage");
	ignorePantryMessage.classList.toggle("hide-element", !this.checked);

	if (this.checked) {
		ignorePantryMessage.textContent =
			"Typical Pantry Items such as water, salt and pepper will be ignored from Missed-Ingredient-Count";
	} else {
		ignorePantryMessage.textContent = "";
	}
});
...
function showRecipeModal(recipeId) {
	...
	if (selectedRecipe) {
		...
		let recipeIdElement = document.createElement("div");
		...
		let recipeMissedIngredientCount = document.createElement("div");
		...
		let recipeMissedIngredients = document.createElement("ul");
...
function displayFavoriteRecipes() {
	...
	if (favoriteRecipes.length > 0) {
		...
		noRecipeMessage.textContent =
			"No favorite recipes found. You have not saved any recipes yet.";
	}
}
